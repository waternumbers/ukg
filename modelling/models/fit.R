...
fitModel <- function(gauge_description, dataPath = "/data/EnvironmentAgency/by_station"){
    ...
    if( dir.exists(gauge_description$gauge) ){return(TRUE)}
    ...
    saveRDS(out,file.path(gauge_description$gauge,"output.rds"))
    ...
    mdl$fit <- NULL
    saveRDS(mdl,file.path(gauge_description$gauge,"model.rds"))
    return(TRUE)
}
